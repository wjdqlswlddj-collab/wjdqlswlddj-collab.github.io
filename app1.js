...
let view='tour',historyTour=null,selectedTeam=C[0],selectedTarget=0,amount=0,vaultAmount=0,busy=false,bracketRound=0,pendingBetResults=[];
function freshTeam(){return{gw:0,r32:0,r16:0,qf:0,sf:0,fin:0,win:0,pts:0,gf:0,ga:0}}
function fresh(){let teams={};C.forEach(c=>teams[c]=freshTeam());return{total:0,last:0,teams:teams,archives:[],wallet:{bal:STARTING_BALANCE,vault:0,interest:0,lastInterest:0,betN:0,hit:0,miss:0,fee:0,profit:0,log:[],vaultLog:[],recoveryTry:0,recoveryHit:0},player:{champStars:0},active:null}}
...
function normalize(x){
  ...
  x.wallet=Object.assign(fresh().wallet,x.wallet||{});x.wallet.log=Array.isArray(x.wallet.log)?x.wallet.log:[];x.wallet.vaultLog=Array.isArray(x.wallet.vaultLog)?x.wallet.vaultLog:[];
  ['bal','vault','interest','lastInterest','betN','hit','miss','fee','profit','recoveryTry','recoveryHit'].forEach(k=>x.wallet[k]=Number(x.wallet[k])||0);
  x.player=Object.assign({champStars:0},x.player||{});x.player.champStars=Math.max(0,Math.floor(Number(x.player.champStars)||0));
  if(x.active){
