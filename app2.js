...
function placeBet(){
  ...
  if(!a||a<10)return alert('최소 베팅금액은 10 BP임');
...
function settle(){
  ...
  a.bets.forEach(b=>{
    ...
    if(decided||!alive){let hit=a.t.reach[SK[b.target]].has(b.team);b.status=hit?'hit':'miss';
      if(hit){b.pay=Math.floor(b.stake*b.odds);S.wallet.bal+=b.pay;S.wallet.hit++;S.wallet.profit+=b.pay-b.stake-b.fee}else{S.wallet.miss++;S.wallet.profit-=b.stake+b.fee}
      S.wallet.log.unshift({n:S.last+1,team:b.team,target:b.target,stake:b.stake,fee:b.fee,odds:b.odds,status:b.status,pay:b.pay,profit:hit?b.pay-b.stake-b.fee:-b.stake-b.fee,opp:b.opp});
...
function prepare(silent){if(busy)return;S.active={t:sim(),reveal:-1,bets:[],locked:null,committed:false,recovery:null};selectedTeam=C.slice().sort((x,y)=>rating(y)-rating(x))[0];selectedTarget=0;amount=0;bracketRound=0;save();if(!silent)render()}
function recoveryPick(){let el=document.getElementById('recoveryTeam'),c=el?el.value:selectedTeam;S.active.recovery=c;S.wallet.recoveryTry++;save();render()}
function revealNext(){
  let a=S.active;if(!a)return;if(a.reveal<0&&S.wallet.bal<130&&!a.recovery)return alert('구제 우승국 예측을 먼저 확정해야 함');if(a.reveal>=5)return;
  a.reveal++;bracketRound=Math.min(Math.max(a.reveal,0),4);settle();
  if(a.reveal===5&&!a.committed){a.committed=true;commit(a.t,true);if(a.recovery){if(a.recovery===a.t.champ){S.wallet.bal+=10000;S.wallet.recoveryHit++;S.wallet.log.unshift({n:S.last,team:a.recovery,target:5,stake:0,fee:0,odds:0,status:'recovery-hit',pay:10000,profit:10000})}else S.wallet.log.unshift({n:S.last,team:a.recovery,target:5,stake:0,fee:0,odds:0,status:'recovery-miss',pay:0,profit:0});S.wallet.log=S.wallet.log.slice(0,100)}save()}else save();
  render();if(pendingBetResults.length)setTimeout(showNextBetResult,30)
}
