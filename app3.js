...
function betPanel(){
  let a=S.active;
  if(!a)return'<div class="card bet-hero"><div class="bet-hero-head"><div><h2>제 '+(S.last+1)+'회 대회 준비</h2><p>대회를 생성한 뒤 조별리그 시작 전부터 목표 라운드 베팅 가능</p></div><div class="balance-pill"><span>보유 베팅 포인트</span><b>'+S.wallet.bal.toLocaleString()+'원</b></div></div><div class="bet-inner"><button class="btn primary" style="width:100%" onclick="prepare()">새 대회 생성</button></div></div>';
  if(a.reveal>=5)return'';
  if(S.wallet.bal<130&&a.reveal<0&&!a.recovery)return'<div class="card bet-hero"><div class="bet-hero-head"><div><h2>구제 우승 예측 모드</h2><p>우승국을 맞히면 10,000원를 지급하며 실패 시 다음 대회에서 재도전 가능</p></div><div class="balance-pill"><span>현재 보유금액</span><b>'+S.wallet.bal.toLocaleString()+'원</b></div></div><div class="bet-inner"><div class="field"><label>우승 예상 국가</label><select id="recoveryTeam">'+C.slice().sort((x,y)=>T[x][0].localeCompare(T[y][0])).map(c=>'<option value="'+c+'">'+T[c][0]+' ('+c+')</option>').join('')+'</select></div><button class="btn gold bet-submit" onclick="recoveryPick()">무료 우승 예측 확정</button></div></div>';
  ...
  return'<div class="card bet-hero"><div class="bet-hero-head"><div><h2>▣ '+stageName(a.reveal)+' · 목표 라운드 베팅</h2><p>누적 진출확률과 다음 상대 전력을 반영하며 동일 대회 반복 베팅에는 누진 수수료 적용</p></div><div class="balance-pill"><span>보유 베팅 포인트</span><b>'+S.wallet.bal.toLocaleString()+'원</b></div></div><div class="bet-inner">'+
...
function treeMatchHtml(m,result,label){
  if(!m)return'<div class="tree-match hiddenmatch">대진 미정</div>';
...
function bracketHtml(t,reveal){
  let labels=['32강','16강','8강','4강','결승'];
  let visible=Math.min(Math.max(reveal,0),4)+1;
  let colW=196,gap=46,canvasH=1120,playTop=54,playH=1030;
  let width=visible*colW+(visible-1)*gap;
  let cols='',paths='';
  for(let r=0;r<visible;r++){
    let round=t.rounds[r]||[],x=r*(colW+gap),result=reveal>=r+1;
    let matches=round.map((m,i)=>{
      let center=Math.pow(2,r)*(2*i+1),y=playTop+(center/32)*playH;
      return'<div class="tree-slot" style="top:'+y.toFixed(2)+'px">'+treeMatchHtml(m,result,labels[r]+' '+(i+1))+'</div>'
    }).join('');
    let status=result?'결과 공개':'대진 확정';
    ...
    if(r<visible-1){
      let curr=t.rounds[r]||[],next=t.rounds[r+1]||[],x1=x+colW,xMid=x1+gap/2,x2=(r+1)*(colW+gap);
      for(let j=0;j<next.length;j++){
        let ca=Math.pow(2,r)*(2*(j*2)+1),cb=Math.pow(2,r)*(2*(j*2+1)+1),cn=Math.pow(2,r+1)*(2*j+1);
  ...
  let third='';
  if(reveal>=5&&t.third){let x=(visible-1)*(colW+gap),y=830;third='<div class="tree-third" style="left:'+x+'px;top:'+y+'px;width:'+colW+'px"><div class="tree-third-title">3·4위전</div>'+treeMatchHtml(t.third,true,'Match 103')+'</div>'}
  return'<div class="bracket-mobile-note">← 좌우로 밀어 전체 대진 확인 →</div><div class="tree-scroll"><div class="tree-canvas" style="width:'+width+'px;height:'+canvasH+'px"><svg class="tree-lines" width="'+width+'" height="'+canvasH+'" viewBox="0 0 '+width+' '+canvasH+'" aria-hidden="true">'+paths+'</svg>'+cols+third+'</div></div>'
}
...
function renderTour(){
  ...
  let a=S.active,s='<div class="sectionhead"><div><h1>제 '+(S.last+1)+'회 '+(a?'월드컵':'대회 준비')+'</h1><p>결과는 조별리그부터 순차 공개되며 공개되지 않은 라운드의 점수는 표시하지 않음</p></div></div>'+betPanel();
